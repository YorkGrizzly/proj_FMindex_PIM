...
#define SAMPLE_RATE 128  // sample rate of occ
...
#define QUERY_NUM 64000  // number of queries
#define READ_NUM 64000  // number of reads
...
int main() {
  ...
  FILE *input_table = fopen("../table_soft_64000.txt", "r");
  ...
  FILE *input_query = fopen("../query_sorted_64000.txt", "r");
  ...
  for(uint32_t i = 0; i < QUERY_NUM; i++){
    if(i % 1000 == 0) printf("QUERY %d found: %d\n", i, num_query_found[i]);
  }
